class SFW_Artifact_Dummy extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_DummySound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_DummyLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_DummyLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPDummy; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_DummyLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_DummyLight()
	{
        ...
        SetDiffuseColor(0.5, 0.0, 0.5);		/* - базовый цвет */
        SetAmbientColor(0.5, 0.0, 0.5);		/* - идущий цвет */
