class SFW_Artifact_Battery extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_BatterySound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_BatteryLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_BatteryLight, "0 0.04 0", 0.0) ); //старая штука
...
class SFW_Artifact_BatteryLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_BatteryLight()
	{
