class SFW_Artifact_FireBall extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_FireBallSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_FireBallLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_FireBallLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPFireBall; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_FireBallLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_FireBallLight()
	{
		SetVisibleDuringDaylight(true);			/* - cвет днем */
		SetRadiusTo( 0.7 );						/* - радиус */
		SetBrightnessTo( 1.7 );					/* - яркость */
