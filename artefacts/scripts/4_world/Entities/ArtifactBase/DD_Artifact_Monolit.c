class SFW_Artifact_Monolit extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            //m_ArtifactSoundName = "SFW_Artifact_MonolitSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_MonolitLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_MonolitLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPMonolit; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_MonolitLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_MonolitLight()
	{
        SetVisibleDuringDaylight(true);			/* - cвет днем */
        SetRadiusTo( 0.5 );						/* - радиус */
        SetBrightnessTo( 2.4 );					/* - яркость */
        SetCastShadow(false);					/* - тени */
        SetFadeOutTime(0.0);					/* - время затухания (секунды) */
        SetDiffuseColor(0.54, 0.0, 1.0);			/* - базовый цвет */
        SetAmbientColor(0.54, 0.0, 1.0);			/* - идущий цвет */
