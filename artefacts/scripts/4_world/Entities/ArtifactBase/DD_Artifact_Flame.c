class SFW_Artifact_Flame extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_FlameSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_FlameLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_FlameLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPFlame; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_FlameLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_FlameLight()
	{
        SetVisibleDuringDaylight(true);			/* - cвет днем */
        SetRadiusTo( 0.8 );						/* - радиус */
        SetBrightnessTo( 2.7 );					/* - яркость */
