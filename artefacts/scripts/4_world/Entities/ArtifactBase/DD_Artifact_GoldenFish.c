class SFW_Artifact_GoldenFish extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_GoldenFishSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_GoldenFishLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_GoldenFishLight, "0 0.04 0", 0.0) ); //старая штука
...
class SFW_Artifact_GoldenFishLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_GoldenFishLight()
	{
