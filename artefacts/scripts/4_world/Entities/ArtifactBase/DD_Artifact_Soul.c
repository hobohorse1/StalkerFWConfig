class SFW_Artifact_Soul extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_SoulSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_SoulLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_SoulLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPSoul; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_SoulLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_SoulLight()
	{
