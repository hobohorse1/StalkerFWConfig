class SFW_Artifact_MomisBeads extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_MomisBeadsSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_MomisBeadsLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_MomisBeadsLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPMom; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_MomisBeadsLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_MomisBeadsLight()
	{
        SetVisibleDuringDaylight(true);					/* - cвет днем */
        SetRadiusTo( 0.4 );								/* - радиус */
        SetBrightnessTo( 1.5 );							/* - яркость */
        SetCastShadow(false);							/* - тени */
        SetFadeOutTime(0.0);							/* - время затухания (секунды) */
        SetDiffuseColor(1.0, 0.0, 0.0);					/* - базовый цвет */
        SetAmbientColor(1.0, 0.0, 0.0);					/* - идущий цвет */
