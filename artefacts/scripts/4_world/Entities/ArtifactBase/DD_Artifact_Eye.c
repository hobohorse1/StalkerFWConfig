class SFW_Artifact_Eye extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_EyeSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_EyeLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_EyeLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPEye; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_EyeLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_EyeLight()
	{
        SetVisibleDuringDaylight(true);			/* - cвет днем */
        SetRadiusTo( 0.2 );						/* - радиус */
        SetBrightnessTo( 1.3 );					/* - яркость */
