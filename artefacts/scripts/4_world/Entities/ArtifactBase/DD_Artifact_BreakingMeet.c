class SFW_Artifact_BreakingMeet extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_BreakingMeetSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_BreakingMeetLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_BreakingMeetLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPMeet; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_BreakingMeetLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_BreakingMeetLight()
	{
