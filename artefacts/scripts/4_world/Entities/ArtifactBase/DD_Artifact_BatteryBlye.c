class SFW_Artifact_BatteryBlue extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_BatteryBlueSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_BatteryBlueLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_BatteryBlueLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPBatteryBlye; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_BatteryBlueLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_BatteryBlueLight()
	{
        SetVisibleDuringDaylight(true);			/* - cвет днем */
        SetRadiusTo( 0.6 );						/* - радиус */
        SetBrightnessTo( 2.1 );					/* - яркость */
        SetCastShadow(false);					/* - тени */
        SetFadeOutTime(0.0);					/* - время затухания (секунды) */
        SetDiffuseColor(0.0, 0.0, 1.0);			/* - базовый цвет */
        SetAmbientColor(0.0, 0.0, 1.0);			/* - идущий цвет */
