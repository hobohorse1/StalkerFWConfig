class SFW_Artifact_Twist extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_TwistSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_TwistLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_TwistLight, "0 0.04 0", 0.0) ); //старая штука
...
class SFW_Artifact_TwistLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_TwistLight()
	{
