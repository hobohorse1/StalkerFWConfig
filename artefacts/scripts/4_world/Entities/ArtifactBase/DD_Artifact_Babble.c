class SFW_Artifact_Bubble extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_BubbleSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_BubbleLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_BubbleLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPBubble; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_BubbleLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_BubbleLight()
	{
