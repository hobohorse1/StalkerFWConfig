class SFW_Artifact_Thorn extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_ThornSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_ThornLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_ThornLight, "0 0.04 0", 0.0) ); //старая штука
...
class SFW_Artifact_ThornLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_ThornLight()
	{
