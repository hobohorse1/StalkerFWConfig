class SFW_Artifact_BloodOfStone extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_BloodOfStoneSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_BloodOfStoneLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_BloodOfStoneLight, "0 0.04 0", 0.0) ); //старая штука
...
class SFW_Artifact_BloodOfStoneLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_BloodOfStoneLight()
	{
