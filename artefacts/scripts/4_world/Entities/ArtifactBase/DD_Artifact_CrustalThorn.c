class SFW_Artifact_CrustalThorn extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_CrustalThornSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_CrustalThornLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_CrustalThornLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPCrustalThorn; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_CrustalThornLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_CrustalThornLight()
	{
