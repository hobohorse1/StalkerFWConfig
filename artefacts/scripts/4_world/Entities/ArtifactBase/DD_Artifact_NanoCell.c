class SFW_Artifact_NanoCell extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            //m_ArtifactSoundName = "SFW_Artifact_NanoCellSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_NanoCellLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_NanoCellLight, "0 0.04 0", 0.0) ); //старая штука
...
class SFW_Artifact_NanoCellLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_NanoCellLight()
	{
