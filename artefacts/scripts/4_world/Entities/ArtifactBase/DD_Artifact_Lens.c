class SFW_Artifact_Lens extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_LensSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_LensLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_LensLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPMoonLight; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_LensLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_LensLight()
	{
        SetVisibleDuringDaylight(true);			/* - cвет днем */
        SetRadiusTo( 0.5 );						/* - радиус */
        SetBrightnessTo( 1.1 );					/* - яркость */
