class SFW_Artifact_Insulator extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            //m_ArtifactSoundName = "SFW_Artifact_InsulatorSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_InsulatorLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_InsulatorLight, "0 0.04 0", 0.0) ); //старая штука
...
class SFW_Artifact_InsulatorLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_InsulatorLight()
	{
