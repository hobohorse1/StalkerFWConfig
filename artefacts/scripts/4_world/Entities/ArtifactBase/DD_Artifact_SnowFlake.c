class SFW_Artifact_SnowFlake extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_SnowFlakeSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_SnowFlakeLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_SnowFlakeLight, "0 0.04 0", 0.0) ); //старая штука
            m_ArtifactLightPos = "0 0.04 0"; //Позиция света на объекте
            m_ArtifactParticleId = ParticleList.SFWPSnowFlake; //какой партикл
            m_ArtifactParticlePos = "0 0.04 0"; // позиция партикла на объекте
        }
    }
}

class SFW_Artifact_SnowFlakeLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_SnowFlakeLight()
	{
        ...
        SetDiffuseColor(0.0, 0.74, 1.0);			/* - базовый цвет */
        SetAmbientColor(0.0, 0.74, 1.0);			/* - идущий цвет */
