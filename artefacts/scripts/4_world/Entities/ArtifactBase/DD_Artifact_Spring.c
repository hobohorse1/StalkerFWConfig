class SFW_Artifact_Spring extends SFW_ArtifactBase
{
    override void ArifactInitialize()
    {
        if(!GetGame().IsMultiplayer() || GetGame().IsClient())
        {
            m_ArtifactSoundName = "SFW_Artifact_SpringSound"; //назввание звука
            m_ArtifactIdleLight = SFW_Artifact_SpringLight.Cast( ScriptedLightBase.CreateLight(SFW_Artifact_SpringLight, "0 0.04 0", 0.0) ); //старая штука
...
class SFW_Artifact_SpringLight extends PointLightBase
{
	static string     m_MemoryPoint = "light";
	void SFW_Artifact_SpringLight()
	{
